...
#define REG_WAVE_CTRL           0x300
#define  REG_WAVE_CTRL_ENABLE   (1 << 0)
#define REG_WAVE_CHANS          0x304
#define REG_WAVE_SMPLBITS       0x308
#define REG_WAVE_SMPLRATE       0x30c
...
static struct {
    ...
    uint32_t wave_ctrl, wave_chans, wave_smplbits, wave_smplrate;
    uint64_t start_us;
    FILE *fp;
    unsigned fpdir;
} vsi[NUM_VSI];
...
static int vsi_read(void *priv, uint64_t addr, size_t len, void *buf, unsigned flags)
{
    ...
    case REG_DMA_BLKIDX:
        *(uint32_t *)buf = vsi[idx].dma_blkidx;
        break;
    case REG_WAVE_CTRL:
        *(uint32_t *)buf = vsi[idx].wave_ctrl;
        break;
    case REG_WAVE_CHANS:
        *(uint32_t *)buf = vsi[idx].wave_chans;
        break;
    case REG_WAVE_SMPLBITS:
        *(uint32_t *)buf = vsi[idx].wave_smplbits;
        break;
    case REG_WAVE_SMPLRATE:
        *(uint32_t *)buf = vsi[idx].wave_smplrate;
        break;
    }
...
static int vsi_write(void *priv, uint64_t addr, size_t len, void *buf, unsigned flags)
{
    ...
    case REG_DMA_BLKNUM:
        ...
        break;
    case REG_WAVE_CTRL:
        vsi[idx].wave_ctrl = val;
        break;
    case REG_WAVE_CHANS:
        vsi[idx].wave_chans = val;
        break;
    case REG_WAVE_SMPLBITS:
        vsi[idx].wave_smplbits = val;
        break;
    case REG_WAVE_SMPLRATE:
        vsi[idx].wave_smplrate = val;
        break;
    }
